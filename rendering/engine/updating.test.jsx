...
Deno.test("use elements => update html elements", async (t) => {
  ...
  await t.step("elements => update elements => updated html", () => {
    const updateHtml = (oldJsx, newJsx) => updateElements(renderElements(oldJsx)[0], newJsx)[0].outerHTML
  ...
  await t.step("factories => update elements => updated elements", () => {
    const updateNames = (oldJsx, newJsx) => updateElements(renderElements(oldJsx)[0], newJsx).map(getHtmlName)
  ...
  await t.step("elements with props => update elements => updated elements props", () => {
    const updateClasses = (oldJsx, newJsx) => updateElements(renderElements(oldJsx)[0], newJsx).map(x => x.className)
  ...
  await t.step("factory effect => update factory => run async effect after all children updated", async () => {
    ...
    const $elem = renderElements(<A><b></b></A>)[0]
    await waitForAsyncs()

    updateElements($elem, <A><c></c></A>)
    await waitForAsyncs()
  ...
  await t.step("element throw error => update element => error dispatched", () => {
    ...
    try { updateElements($elem, <a onerror={handler}><B></B></a>) }
